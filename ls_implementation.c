...
//opens each directory in the given directory and shows contents
void show_each_dir(){
...
void show_dir_files(char *dir, struct options opts) {
    ...
    //char **hidden_dir_arr = (char **) calloc((size_t) calloc_size, sizeof(char *));
    ...
    if (current_dir_index)
        file_information(dir, dir_arr[current_dir_index], opts, &is_dir_array[current_dir_index]);
    else {
        for (long int i = 0; i < cur_dir; ++i){
            file_information(dir, dir_arr[i], opts, &is_dir_array[i]);
        }
    }
    //=============================================
   /*
    for (long int i = 0; i < cur_dir; ++i)
    {
        file_information(dir, hidden_dir_arr[i], opts, &is_dir_array[i]);
    }
    */
    // ===============================================
    /*for(long int i =0; i<cur_dir;++i)
...
void run_ls(int count, char* argv[], struct options options) {
    ...
    if(options.l||options.n||options.i||options.a){
        show_dir_files(".", options);
    }

    if(options.R){
        //for(int i = optind; i < count; ++i)
        show_each_dir(".", options);
    }
}
